...
![Chess app screenshot](https://github.com/user-attachments/assets/26c94323-a96d-45f5-bda0-9abec82572bd)
...
The code is cleanly modularized into separate areas of concern to make it easy to maintain:
...
│   ├── index.css                   # Global CSS (imports Tailwind CSS & fonts)
│   ├── components/
│   │   └── board/
...
Example payload transmitted from the client:
...
2. **Format play history:** Translates the move history list into official game score format (e.g., `1. e4 e5 2. Nf3 Nc6`).
...
4. **Schema enforcement:** To ensure instant rendering without manual parsers, the server requests a strictly validated JSON response:
...
To unlock full grandmaster-level coaching feedback, create a `.env` file at the root of your project (or set the variable in your environment):
